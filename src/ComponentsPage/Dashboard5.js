...
const Dashboard5 = () => {
  ...
  const {
    ...
    queryFn: () =>
      getPlannedValue(selectedYear), // API call to fetch data based on year selected
    enabled:
      Boolean(selectedYear), // Only run query if year are selected
  });
  ...
  const {
    ...
    queryFn: () =>
      getUnplannedValue(selectedYear), // API call to fetch data based on year selected
    enabled:
      Boolean(selectedYear), // Only run query if year are selected
  });
